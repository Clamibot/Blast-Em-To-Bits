...
namespace SpaceGraphicsToolkit
{
	...
	public class SgtStarfieldNearTex_Editor : SgtEditor<SgtStarfieldNearTex>
	{
		protected override void OnInspector()
		{
			...
			EndError();
			Draw("format", ref updateTexture, "The texture format of the generated texture.");

			Separator();

			Draw("ease", ref updateTexture, "The transition style.");
			Draw("sharpness", ref updateTexture, "The sharpness of the transition.");
			BeginError(Any(t => t.Offset >= 1.0f));
				Draw("offset", ref updateTexture, "The start point of the fading.");
			EndError();

			if (updateTexture == true) DirtyEach(t => t.DirtyTexture());
		}
	}
}
#endif