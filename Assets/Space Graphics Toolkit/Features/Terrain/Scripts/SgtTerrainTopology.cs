...
namespace SpaceGraphicsToolkit
{
	...
	public static class SgtTerrainTopology
	{
		...
		public static Color Tex2D_Point(NativeArray<Color32> pixels, int2 size, double2 uv)
		{
		...
		public static Color Sample(NativeArray<Color32> pixels, int2 size, long x, long y)
		{
			x = (x % size.x + size.x) % size.x;
			y = (y % size.y + size.y) % size.y;
		...
		{
			var s  = size / new double2(math.PI * 2.0, math.PI);
			var u  = (math.PI * 2.0 - math.atan2(direction.x, direction.z)) * s.x;
			var v  = (math.asin(direction.y) + math.PI * 0.5) * s.y;
			var uv = new double2(u, v);
